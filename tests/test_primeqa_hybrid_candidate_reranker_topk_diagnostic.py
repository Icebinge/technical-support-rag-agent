...
def test_primeqa_hybrid_topk_diagnostic_runs_train_dev_only(tmp_path):
    ...
    visualizations = write_primeqa_hybrid_candidate_reranker_topk_diagnostic_visualizations(
        run=run,
        output_dir=tmp_path / "visuals",
    )

    assert run.report["stage"] == "Stage 73"
    assert run.report["topk_diagnostic"]["max_answer_candidates"] == 2
    assert run.report["topk_diagnostic"]["topk_mode_name"] == (
        "top2_leading_candidate_rewrite"
    )
    assert [
        row["model_name"]
        for row in run.report["topk_diagnostic"]["policy_rows"]
        if row["policy_label"] == "candidate_score_gte_60"
    ...
    assert run.report["decision"]["default_runtime_policy"] == "unchanged"
    assert len(visualizations) == 20
    assert "stage73_logistic_best_candidate_dev_holdout_top2_policy_delta.svg" in {
        artifact.name for artifact in visualizations
    }


def test_primeqa_hybrid_topk_diagnostic_blocks_test_rows(tmp_path):
    ...
    assert run.report["decision"]["status"] == (
        "primeqa_hybrid_candidate_reranker_topk_diagnostic_blocked"
    )
    ...
    return {
        ...
        "candidate_artifact_summary": {
            "summary": {
                ...
                "total_questions": sum(
                    len(question_ids)
                    for question_ids in question_ids_by_split.values()
                ),
                "total_rows": len(rows),
                "questions_by_split": {
