...
def test_runtime_document_risk_proxy_search_uses_runtime_features_only():
    rows = [
        candidate
    ...
    assert {
        evaluation.feature_scope for evaluation in result.guard_evaluations
    } == {"runtime_available_features_only"}
    assert evaluation_by_label[RUNTIME_PROXY_GUARD_LABEL].probe_cases
    assert result.findings
    assert result_dict["analysis_scope"].startswith("Offline Stage 43")


def test_runtime_document_risk_proxy_search_writes_visualizations(tmp_path):
    rows = [
        candidate
        for question_index in range(1, 7)
        for candidate in _question_rows(question_index)
...
def test_runtime_document_risk_proxy_search_validates_stage42_report():
    rows = [
        candidate
        for question_index in range(1, 7)
        for candidate in _question_rows(question_index)
    ...
    gold_answers = _gold_answers(rows)
    stage42_report = _stage42_report(rows, gold_answers)
    stage42_report["mode_name"] = "different_mode"
    ...
    try:
        ...
        assert "mode_name does not match" in str(exc)
    else:
        raise AssertionError("mismatched Stage 42 report should fail")
