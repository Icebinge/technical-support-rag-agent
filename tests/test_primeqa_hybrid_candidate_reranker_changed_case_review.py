...
def test_primeqa_hybrid_changed_case_review_runs_public_safe_train_dev_only(tmp_path):
    paths = _write_fixture(tmp_path, include_test_candidate=False)
    stage71_report = _write_stage71_report(paths, tmp_path)

    report = review_primeqa_hybrid_candidate_reranker_changed_cases(
        stage71_report_path=stage71_report,
    ...
    assert report["stage"] == "Stage 72"
    assert [review["model_name"] for review in report["model_reviews"]] == [
        "logistic_best_candidate",
        "ridge_candidate_token_f1",
    ]
    assert all(check["passed"] for check in report["guard_checks"])
    assert report["decision"]["can_run_final_test_metrics_now"] is False
    assert "Correct service fix" not in serialized
    assert "Original weak answer" not in serialized
    assert {artifact.name for artifact in visualizations} == {
        "stage72_dev_top3_delta_by_policy.svg",
        "stage72_changed_cases_by_policy.svg",
        "stage72_candidate_score_vs_main_outcomes.svg",
        "stage72_residual_regressions_by_policy.svg",
    }


def test_primeqa_hybrid_changed_case_review_blocks_test_rows(tmp_path):
    ...
    assert report["decision"]["status"] == (
        "primeqa_hybrid_candidate_reranker_changed_case_review_blocked"
    )


def _write_stage71_report(paths: dict[str, Path], tmp_path: Path) -> Path:
    stage71_run = run_primeqa_hybrid_candidate_reranker_development(
        candidate_dataset_path=paths["candidate_dataset"],
    ...
    path = tmp_path / "stage71.json"
    path.write_text(
        json.dumps(stage71_run.report, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    return path
