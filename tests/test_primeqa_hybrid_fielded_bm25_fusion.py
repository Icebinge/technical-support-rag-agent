...
def test_fielded_bm25_fusion_runs_train_dev_only_and_public_safe(tmp_path):
    ...
    report = run_primeqa_hybrid_fielded_bm25_fusion(
        ...
        stage77_report_path=paths["stage77_report"],
        top_k_values=(1, 5, 10),
        candidate_depth=10,
    )
    ...
    assert report["stage"] == "Stage 78"
    assert set(report["metrics_by_split"]) == {"dev", "train"}
    assert report["train_selection"]["selected_config_id"].startswith("fielded_")
    ...
    assert {artifact.name for artifact in visualizations} == {
        "stage78_fielded_bm25_train_hit_at_10.svg",
        "stage78_fielded_bm25_dev_hit_at_10.svg",
        "stage78_fielded_bm25_dev_delta_hit_at_10.svg",
        "stage78_fielded_bm25_dev_top10_changes.svg",
    }


def test_fielded_bm25_fusion_blocks_stage75_baseline_mismatch(tmp_path):
    paths = _write_fixture(tmp_path)
    ...
    assert report["decision"]["status"] == "primeqa_hybrid_fielded_bm25_fusion_blocked"
...
def _write_fixture(tmp_path: Path) -> dict[str, Path]:
    ...
    stage77_report = tmp_path / "stage77.json"
    stage77_report.write_text(
        json.dumps(
            {
                "stage": "Stage 77",
