...
def test_query_view_ablation_runs_train_dev_only_and_public_safe(tmp_path):
    ...
    assert report["stage"] == "Stage 77"
    assert set(report["metrics_by_split"]) == {"dev", "train"}
    assert report["train_selection"]["selected_view_id"] in {
    ...
    assert {artifact.name for artifact in visualizations} == {
        "stage77_query_view_train_hit_at_10.svg",
        "stage77_query_view_dev_hit_at_10.svg",
        "stage77_query_view_dev_delta_hit_at_10.svg",
        "stage77_query_view_dev_top10_changes.svg",
    }


def test_query_view_ablation_blocks_stage75_baseline_mismatch(tmp_path):
    paths = _write_fixture(tmp_path)
    stage75 = json.loads(paths["stage75_report"].read_text(encoding="utf-8"))
    stage75["split_reports"]["dev"]["hit_at_top_k"] = 0.0
    ...
    assert checks["baseline_dev_hit10_matches_stage75"]["passed"] is False
    assert report["decision"]["status"] == "primeqa_hybrid_query_view_ablation_blocked"
    ...
    _write_jsonl(
        ...
        [
            _split_sample(
                ...
                question_text="restart restart restart service admin",
    ...
    _write_jsonl(
        ...
        [
            _split_sample(
                ...
                question_text="install install package failure error",
        ...
        json.dumps(
            {
                ...
                "doc-decoy": {
                    ...
                    "text": "install install package failure error",
                    "sections": [],
                },
            },
            ensure_ascii=False,
