...
Stage91 should confirm and freeze the train/dev-only protocol for
`section_signal_guarded_expansion_design`. The frozen test split remains locked,
final metrics must not be run, source `DOC_IDS` must not be used as runtime
retrieval evidence, and runtime defaults remain unchanged.
