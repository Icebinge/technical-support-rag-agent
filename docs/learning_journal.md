...
## 2026-07-18 - Stage 160 全量开发集失败链路与延迟诊断

目标：在完全不打开 test、不改默认 runtime、不训练或选择策略的前提下，对 Stage159 已冻结的 121 条 warm 多轮真实 HTTP workload 加入 validation-only 观察器。开发集 gold 只在真实 turn 结束后用于判断损失发生在候选池、generation Top10、模型决策还是 composition；gold 不进入 retrieval、router、prompt、composition 或 verification 的运行输入。

### 冻结协议

唯一数据源仍是 Stage68 dev 文件，SHA-256 为 `071c54f80657592bda7f8e4095afc8800a2be112362c3a275191a0fc8e28bd5f`，共 121 行，其中 answerable 76、unanswerable 45。继续复用 Stage159 排序 SHA-256 `3b8a39cae397db4402080a2780178ade0fd4fc3a9ba5facb25d041510e8b69b7` 和 30 个四轮加 1 个单轮的 synthetic workload，分组 SHA-256 为 `7aa271a775c2926b32226e0a4fccc96cff3a7bf98fc90246c8002d79561fd6d0`。这些 thread 是压测分组，不是真实自然会话。

按用户要求加入 grouped five-fold 稳定性分析。分组键是 `normalized question + answer document / UNANSWERABLE`，121 行形成 117 组；各 fold 行数为 `25/24/24/24/24`，组数为 `24/23/23/23/24`，assignment SHA-256 为 `79c5b8d805d00b36bb653bc1243b91985caf923b8c68ee39cfe54d48cd51f739`。这里没有 fit model、select policy 或 tune threshold，因此它是五折诊断稳定性，不是用 dev 训练的交叉验证。

### 实现与边界

新增 Stage160 protocol、validation-only observer、独立 CLI 和两组测试。observer 装饰精确的 Stage158 runtime，在每个真实模型决策后读取候选池、generation Top10、verification context、最终 action/citation 和 latency；生产 runtime 与 HTTP 协议没有修改。

公开报告只保存 aggregate。ignored private artifact 保存 121 行哈希身份与数值/枚举诊断，不保存 raw question、answer、document id、document text、citation id 或 raw model output。复算结果为 121 个合法且唯一的 sample SHA-256、121 个合法 query SHA-256（117 个唯一）、117 个 diagnostic group、76 个 answer document SHA-256（71 个唯一）。规范化 JSON 哈希与公开声明一致。

严格关闭状态：train 未加载、test 未加载、test metrics 未运行；dev 未用于模型拟合、策略选择或阈值调优；runtime default、remote、persistence、query rewrite、second retrieval、queue、retry、fallback 均未打开。

### 正式 GPU 与 HTTP 结果

正式进程只启动一次，使用 loopback `18160`。启动命令没有打印 PID，因此没有据此假设成功，也没有重复启动；只读检查确认 wrapper/child Python 进程、listener 和 workload 进度。进程随后自然跑完 31 个 thread、121 个 turn、报告生成和 shutdown，exit code 为 0，相关进程与 listener 最终均为 0。
...
```text
answerable refusal: 52 / 76 = 68.4211%
unanswerable refusal: 35 / 45 = 77.7778%
unanswerable false answer: 10 / 45 = 22.2222%
answerable gold candidate-pool hit: 70 / 76 = 92.1053%
answerable gold generation-Top10 hit: 36 / 76 = 47.3684%
answerable gold verification-context hit: 69 / 76 = 90.7895%
answerable gold citation: 15 / 76 = 19.7368%
answerable token F1, all answerable: 0.088213
answerable token F1, completed answerable: 0.279342
```

verification-context 是独立的 runtime 输入观察，不能用它反推 generation prompt 也包含相同 gold。定位模型决策损失时以 generation Top10 边界为准。

52 个 answerable refusal 的互斥分解：

```text
候选池没有 gold: 5
候选池有 gold、generation Top10 丢失: 28
generation Top10 可见 gold、模型仍拒答: 19
compose 后拒答: 0
```

这是本阶段最重要的进展：第一阶段候选池已经覆盖 92.1% 的 answerable gold，当前最大损失不是慢速重建更大的候选池，而是第二阶段送给生成器的 Top10 只保留 47.4%。28 个拒答发生在这一层，另有 19 个需要单独研究“证据可见但模型拒答”。

五折中 generation-Top10 answerable hit rate 为 `0.4375-0.5000`，population stddev `0.023206`；answerable refusal rate 为 `0.600000-0.764706`。主要失效机制跨 fold 稳定存在，但这些 fold 没有用于选择任何方案。
...
generation share: 93.0286%
超过 Stage159 p95: 2 / 121
```

generation latency 的 Spearman 相关为：router input tokens `0.915809`、retained state bytes `0.627997`、synthetic turn position `0.566652`、output tokens `0.034743`、固定 candidate count `0.0`。它们是观测相关，不是因果证明。第 1 到 4 轮平均 generation latency 为 `1264.680 / 2007.382 / 3788.273 / 5881.591ms`；两个超过 Stage159 p95 的请求都在第 4 轮。

正式 workload 用时 `419.539637s`，总用时 `484.320761s`。正式 stderr 非空，共 490 bytes，是成功的 model weight-loading progress；没有 traceback，不能写成 stderr empty，也不能把非空 stderr 误报为运行失败。

### 可视化、失败过程与修正

生成 10 张 SVG，覆盖 action、action latency、answerable refusal flow、failure buckets、五折 refusal rate、57 guards、latency correlations、quality rates 和两个 turn-position latency 视图。10/10 均可 XML parse，具有明确的 `1480` 宽度、viewBox、title、text 和 bar 元素。

当前本地图像查看器不能处理 SVG；随后应用内浏览器因 URL 安全策略拒绝打开本地 `file://`，没有绕过策略。因此可以声明 XML 与结构检查通过，但不能声称做过逐像素 screenshot review。

本阶段保留以下真实失败与修正：

- 第一次并行只读 audit 因一个 `rg` 无匹配而 exit 1；另一次 Windows wildcard `rg` 报 invalid filename syntax。两者都没有改文件。
- 第一次 query uniqueness 命令使用当前 .NET 不支持的静态 `SHA256.HashData`，伴随错误打印了无效的 0。改用 `SHA256.Create` 后才得到真实的 117 个 query digest、4 个重复组、8 个重复行、76/45 answerability 和 71 个文档。
- 第一次 Ruff 格式化 3 个文件后报告 5 项问题；修正后还剩 1 项 import sorting，交给 Ruff 修正后通过。
- 第一次 focused pytest 是 `14 passed, 4 failed`。3 项来自测试把内存 LF hash 与 Windows 实际写出的 CRLF bytes 比较；1 项来自手工写错 query digest。测试改为 hash 实际 bytes 并调用 protocol digest 后，结果为 `18 passed`；Stage157-160 扩展集为 `70 passed, 1 existing warning`。
- 文档后的第一条验证命令误把 `ruff format --check` 扩到整个历史仓库，报告当前 formatter 会改 311 个旧文件，然后在 lint 和 pytest 前 exit 1。该命令没有修改文件；正确范围是 Stage160 新增 5 个 Python 文件做 format check、全仓做 Ruff lint，两项均通过。
- 下一条 targeted pytest 命令沿用了两个不存在的旧测试文件名，Ruff 已通过，但 pytest 在 collection 前以 `0 tests`、exit 1 结束。读取真实测试文件列表后，精确运行 Stage157-160 的 9 个文件，得到 `70 passed, 1 existing warning in 2.00s`。
- 正式启动没有返回 PID 文本；未重启，使用只读 process/port/progress/exit-file 观察同一次进程自然结束。
- 第一次读取公开报告输出过长被工具截断，随后改成只读指定 aggregate 字段。第一次 private hash 检查误写字段名 `query_sha256`，打印了假的 0；改为真实字段 `query_digest_sha256` 后验证 121/121 合法。
- `view_image` 不能处理 SVG，应用内浏览器又因安全策略拒绝本地 URL；没有尝试策略绕过。
...
Warning 是既有 FastAPI `TestClient` 的 Starlette deprecation warning。完整 pytest 使用独立 driver 启动，不设置运行期限，只读等待到自然结束；启动命令仍没有打印 PID 文本，因此没有重复启动，实际 wrapper/child 进程与 exit file 均被核对。

公开 artifact SHA-256：`e17e5fe5bbc5fef4e25e41234e47b89daf19ea4ef18f3c7270601f0fee7d9377`。

ignored private artifact byte SHA-256：`3f10cffe245a4405dfc56044f2a3c0d364fdd0f8723e6cc3ae401260199652db`。

private canonical JSON SHA-256：`1c8aa4260be5427e13322cb3304e518dd3609c2e38f839cda4f10ce01c911a0d`。

### 本步学到

- 候选池高召回不等于最终上下文高召回；本次 92.1% 在 generation Top10 处掉到 47.4%，第二阶段筛选才是当前主瓶颈。
- “gold 可见但拒答”与“gold 被筛掉”是两类问题，不能用同一个策略混在一起调。
- 五折不一定意味着训练；当前 grouped folds 只检验失效规律是否稳定，避免用 dev 做 fit/selection。
- 延迟的 93% 来自 generation，且 input token 与 generation latency 的相关最高。下一轮性能设计应首先控制进入模型的上下文，而不是继续优化固定大小候选池的构建。
- 私有逐题诊断可以只保存哈希与数值，并由公开 artifact 声明 canonical hash；这样既能复查规律，又不会把原始问题和文档提交到仓库。
