...
app = typer.Typer(help="Run the Stage 204 train-only Top-1 objective failure attribution.")
...
def main(
    ...
    output_path = (
        output
        or artifacts / "primeqa_hybrid_top1_joint_objective_failure_attribution_stage204.json"
    )
    preflight_failure_path = artifacts / "stage204_stage203_reproduction_failure.json"
    report = analysis.run_stage204_top1_joint_objective_failure_attribution(
        stage203_report_path=artifacts / "primeqa_hybrid_top1_joint_objective_stage203.json",
        ...
        preflight_failure_sink=lambda failure: analysis.write_stage204_preflight_failure(
            report=failure,
            output_path=preflight_failure_path,
        ),
    )
    report = analysis.write_stage204_report_bundle(
        report=report,
        output_path=output_path,
        visualization_dir=visualization_dir
        or artifacts / "primeqa_hybrid_top1_joint_objective_failure_attribution_stage204_visuals",
    )
    typer.echo(json.dumps(_summary(report), ensure_ascii=True, indent=2))
    typer.echo(f"Saved Stage 204 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "stage": report["stage"],
        "stage203_reproduction": report["stage203_reproduction"],
        "population": attribution["population"],
        "control_to_custom_aggregate": attribution["control_to_custom"]["aggregate"],
        "precision_adjacent_aggregate": attribution["precision_adjacent_attribution"]["aggregate"],
        "safety_adjacent_aggregate": attribution["safety_adjacent_attribution"]["aggregate"],
        "target_mechanics": attribution["target_mechanics"],
