...
app = typer.Typer(help="Run the Stage 194 train-only safety-constrained LambdaMART CV.")
...
def main(
    ...
    allow_below_frozen_memory_threshold: Annotated[
        bool, typer.Option("--allow-below-frozen-memory-threshold")
    ] = False,
) -> None:
    settings = ProjectSettings()
    artifacts = settings.artifact_dir
    dependency_dir = artifacts / "stage194_dependency"
    report = analysis.run_stage194_safety_constrained_lambdamart_cv(
        stage193_protocol_path=artifacts
        / "primeqa_hybrid_safety_constrained_lambdamart_protocol_stage193.json",
        ...
        allow_below_frozen_memory_threshold=allow_below_frozen_memory_threshold,
        memory_override_note=(
            "User explicitly authorized proceeding below the frozen 6 GiB preflight "
            "threshold on 2026-07-27; no model-grid reduction or fallback was authorized."
            if allow_below_frozen_memory_threshold
            else ""
        ),
        progress_sink=_progress,
    )
    visualizations = analysis.write_stage194_visualizations(
        report=report,
        output_dir=visualization_dir
        or artifacts / "primeqa_hybrid_safety_constrained_lambdamart_stage194_visuals",
    )
    report = {
        **report,
        "visualizations": [{"name": item.name, "path": item.path} for item in visualizations],
    }
    output_path = output or artifacts / "primeqa_hybrid_safety_constrained_lambdamart_stage194.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(report, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
    typer.echo(json.dumps(_summary(report), ensure_ascii=True, indent=2))
    typer.echo(f"Saved Stage 194 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "selected_profile_counts": cv["selected_profile_counts"],
        "selected_penalty_counts": cv["selected_penalty_counts"],
