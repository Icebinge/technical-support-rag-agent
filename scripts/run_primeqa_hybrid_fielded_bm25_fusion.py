...
from ts_rag_agent.application.primeqa_hybrid_fielded_bm25_fusion import (
    run_primeqa_hybrid_fielded_bm25_fusion,
    write_primeqa_hybrid_fielded_bm25_fusion_visualizations,
)
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(
    help="Run Stage78 train/dev-only PrimeQA hybrid fielded BM25 fusion."
)
...
def main(
    ...
    stage77_report: Annotated[
        Path | None,
        typer.Option("--stage77-report", help="Stage77 query-view ablation report JSON."),
    ] = None,
    output: Annotated[
        Path | None,
        typer.Option("--output", help="Output Stage78 report JSON path."),
    ] = None,
    ...
    candidate_depth: Annotated[
        int,
        typer.Option("--candidate-depth", help="Per-field BM25 candidate depth."),
    ] = 100,
    bm25_k1: Annotated[
        float,
    ...
    """Run Stage78 fielded BM25 fusion and write the report."""
    ...
    stage77_report_path = stage77_report or (
    ...
    visualization_output_dir = visualization_dir or (
        settings.artifact_dir / "primeqa_hybrid_fielded_bm25_fusion_stage78_visuals"
    )

    report = run_primeqa_hybrid_fielded_bm25_fusion(
        train_split_path=train_path,
        dev_split_path=dev_path,
        documents_path=documents_path,
        stage75_report_path=stage75_report_path,
        stage77_report_path=stage77_report_path,
        top_k_values=_parse_top_k_values(top_k),
        candidate_depth=candidate_depth,
        bm25_k1=bm25_k1,
        bm25_b=bm25_b,
    )
    visualizations = write_primeqa_hybrid_fielded_bm25_fusion_visualizations(
        report=report,
    ...
    typer.echo(f"Saved PrimeQA hybrid fielded BM25 fusion report: {output_path}")
...
def _parse_top_k_values(raw_value: str) -> tuple[int, ...]:
    ...
    if 10 not in values:
        raise typer.BadParameter("--top-k must include 10 for Stage78.")
    return values
...
def _console_summary(report: dict[str, Any]) -> dict[str, Any]:
    return {
        "stage": report["stage"],
        "fusion_configs": report["fusion_configs"],
