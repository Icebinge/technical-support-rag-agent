...
from ts_rag_agent.application import (
    primeqa_hybrid_composition_f1_representation_cv as analysis,
)
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(help="Run the Stage184 train-only composition F1 representation CV.")
...
def main(
    ...
    report = analysis.run_stage184_composition_f1_representation_cv(
    ...
    visuals = analysis.write_stage184_visualizations(
        report=report,
        output_dir=visualization_dir
        or artifacts / "primeqa_hybrid_composition_f1_representation_stage184_visuals",
    )
    report = {
        **report,
        "visualizations": [
    ...
    output_path = output or artifacts / "primeqa_hybrid_composition_f1_representation_stage184.json"
    ...
    typer.echo(f"Saved Stage184 report: {output_path}")
    ...
    return {
        ...
        "representations": {
            name: {
                ...
                "stage182_regression_headroom": row["stage182_regression_headroom"],
                "feature_count": row["feature_count"],
                "model_fit_count": row["model_fit_count"],
                "fit_and_predict_seconds": row["fit_and_predict_seconds"],
            }
            for name, row in cv["representations"].items()
        },
