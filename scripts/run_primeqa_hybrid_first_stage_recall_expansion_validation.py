...
app = typer.Typer(
    help="Run Stage124 PrimeQA hybrid first-stage recall expansion validation."
)


@app.command()
def main(
    stage123_protocol: Annotated[
        Path | None,
        typer.Option("--stage123-protocol", help="Stage123 protocol JSON."),
    ] = None,
    ...
    output: Annotated[
        Path | None,
        typer.Option("--output", help="Output Stage124 report JSON path."),
    ] = None,
    ...
    user_confirmed_validation: Annotated[
        ...
        typer.Option(
            ...
            help="Required confirmation for Stage124 train/dev validation.",
        ),
    ] = False,
    ...
    """Write the Stage124 train/dev-only first-stage recall expansion report."""

    settings = ProjectSettings()
    split_dir = settings.artifact_dir / "primeqa_hybrid_split_stage68_splits"
    stage123_protocol_path = stage123_protocol or (
    ...
    visualization_output_dir = visualization_dir or (
        settings.artifact_dir
        / "primeqa_hybrid_first_stage_recall_expansion_validation_stage124_visuals"
    )
    ...
    typer.echo(f"Saved PrimeQA hybrid Stage124 validation: {output_path}")
