...
from ts_rag_agent.application import primeqa_hybrid_composition_dual_target_cv as analysis
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(help="Run the Stage182 train-only nested dual-target composition CV.")
...
def main(
    ...
    report = analysis.run_stage182_composition_dual_target_cv(
    ...
    visuals = analysis.write_stage182_visualizations(
        report=report,
        output_dir=visualization_dir
        or artifacts / "primeqa_hybrid_composition_dual_target_stage182_visuals",
    )
    report = {
        **report,
        "visualizations": [
    ...
    output_path = output or artifacts / "primeqa_hybrid_composition_dual_target_stage182.json"
    ...
    typer.echo(f"Saved Stage182 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "stage": report["stage"],
        "stage181_reproduction": report["stage181_reproduction"],
        "dual_target_nested_cv": report["dual_target_nested_cv"],
        "stage181_single_target_benchmark": report["stage181_single_target_benchmark"],
