...
app = typer.Typer(help="Run the Stage 189 train-only gain-sensitive failure attribution.")
...
def main(
    ...
    report = analysis.run_stage189_gain_sensitive_failure_attribution(
        stage188_report_path=artifacts / "primeqa_hybrid_gain_sensitive_ranking_stage188.json",
        stage187_protocol_path=artifacts
        / "primeqa_hybrid_gain_sensitive_ranking_protocol_stage187.json",
    ...
    visualizations = analysis.write_stage189_visualizations(
        ...
        or artifacts / "primeqa_hybrid_gain_sensitive_failure_stage189_visuals",
    )
    report = {
        **report,
        "visualizations": [
    ...
    output_path = output or artifacts / "primeqa_hybrid_gain_sensitive_failure_stage189.json"
    ...
    typer.echo(f"Saved Stage 189 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "stage": report["stage"],
        "stage188_reproduction": report["stage188_reproduction"],
        "top_ineligible_trajectory": attribution["top_ineligible_trajectory"],
        "factor_aggregates": attribution["factor_aggregates"],
        "family_best_configurations": attribution["family_best_configurations"],
        "diagnostic_findings": attribution["diagnostic_findings"],
        "stage190_protocol": report["stage190_protocol"],
