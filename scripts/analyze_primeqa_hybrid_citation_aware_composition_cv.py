...
from ts_rag_agent.application import primeqa_hybrid_citation_aware_composition_cv as analysis
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(help="Run Stage180 train-only nested citation-aware composition CV.")
...
def main(
    ...
    report = analysis.run_stage180_citation_aware_composition_cv(
    ...
    visuals = analysis.write_stage180_visualizations(
        report=report,
        output_dir=visualization_dir
        or artifacts / "primeqa_hybrid_citation_aware_composition_stage180_visuals",
    )
    report = {**report, "visualizations": [asdict_visual(row) for row in visuals]}
    output_path = output or artifacts / "primeqa_hybrid_citation_aware_composition_stage180.json"
    ...
    typer.echo(f"Saved Stage180 report: {output_path}")
    if report["decision"]["status"] == "stage180_citation_aware_composition_invalid":
        raise typer.Exit(code=1)


def asdict_visual(row: analysis.Stage180Visualization) -> dict[str, str]:
