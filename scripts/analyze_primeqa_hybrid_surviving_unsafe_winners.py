...
app = typer.Typer(help="Run the Stage 197 train-only surviving unsafe-winner attribution.")
...
def main(
    ...
    report = analysis.run_stage197_surviving_unsafe_winner_attribution(
        stage196_report_path=artifacts / "primeqa_hybrid_safety_first_frontier_stage196.json",
    ...
    visualizations = analysis.write_stage197_visualizations(
        ...
        or artifacts / "primeqa_hybrid_surviving_unsafe_winner_stage197_visuals",
    )
    report = {
        **report,
        "visualizations": [{"name": item.name, "path": item.path} for item in visualizations],
    }
    output_path = output or artifacts / "primeqa_hybrid_surviving_unsafe_winner_stage197.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(report, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
    typer.echo(json.dumps(_summary(report), ensure_ascii=True, indent=2))
    typer.echo(f"Saved Stage 197 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "unsafe_head_prediction_metrics": attribution["unsafe_head_prediction_metrics"],
