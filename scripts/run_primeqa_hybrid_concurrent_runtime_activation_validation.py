...
from ts_rag_agent.application.primeqa_hybrid_concurrent_runtime_activation_validation import (
    run_primeqa_hybrid_concurrent_runtime_activation_validation,
    write_primeqa_hybrid_concurrent_runtime_activation_validation_visualizations,
)
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(help="Run Stage146 explicit concurrent application activation validation.")


@app.command()
def main(
    stage145_validation: Annotated[Path | None, typer.Option("--stage145-validation")] = None,
    ...
    """Write the aggregate-only Stage146 report and visualizations."""

    settings = ProjectSettings()
    artifact_dir = settings.artifact_dir
    split_dir = artifact_dir / "primeqa_hybrid_split_stage68_splits"
    report = run_primeqa_hybrid_concurrent_runtime_activation_validation(
        stage145_validation_path=stage145_validation
        or artifact_dir / "primeqa_hybrid_concurrent_runtime_validation_stage145.json",
    ...
    visualizations = write_primeqa_hybrid_concurrent_runtime_activation_validation_visualizations(
        ...
        or artifact_dir
        / "primeqa_hybrid_concurrent_runtime_activation_validation_stage146_visuals",
    )
    report = {
        **report,
        "visualizations": [
    ...
    output_path = output or (
        artifact_dir / "primeqa_hybrid_concurrent_runtime_activation_validation_stage146.json"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    typer.echo(json.dumps(_console_summary(report), ensure_ascii=False, indent=2))
    typer.echo(f"Saved PrimeQA hybrid Stage146 activation validation: {output_path}")
