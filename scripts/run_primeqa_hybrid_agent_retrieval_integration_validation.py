...
from ts_rag_agent.application.primeqa_hybrid_agent_retrieval_integration_validation import (
    run_primeqa_hybrid_agent_retrieval_integration_validation,
    write_primeqa_hybrid_agent_retrieval_integration_validation_visualizations,
)
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(
    help="Run Stage129 PrimeQA hybrid agent retrieval integration validation."
)
...
def main(
    ...
    output: Annotated[
        Path | None,
        typer.Option("--output", help="Output Stage129 report JSON path."),
    ] = None,
    ...
    user_confirmed_validation: Annotated[
        ...
        typer.Option(
            ...
            help="Required confirmation for Stage129 train/dev validation.",
        ),
    ] = False,
    ...
    """Write the Stage129 train/dev-only agent integration validation report."""
    ...
    visualization_output_dir = visualization_dir or (
        settings.artifact_dir
        / "primeqa_hybrid_agent_retrieval_integration_validation_stage129_visuals"
    )

    report = run_primeqa_hybrid_agent_retrieval_integration_validation(
    ...
    typer.echo(f"Saved PrimeQA hybrid Stage129 validation: {output_path}")


def _console_summary(report: dict[str, Any]) -> dict[str, Any]:
    profile_reports = report.get("profile_reports") or {}
    stage128_profile = profile_reports.get("stage128_prefix_append_top400_agent_pool")
    train_cv = report.get("train_cv_validation") or {}
    return {
        "stage": report["stage"],
        "analysis_id": report["analysis_id"],
        "candidate_pool_summary": report.get("candidate_pool_summary"),
        "stage128_train_cv_verified": (
            stage128_profile["split_reports"]["train_cv"]["verified_metrics"]
            if stage128_profile
            else None
        ),
        "stage128_dev_verified": (
            stage128_profile["split_reports"]["dev"]["verified_metrics"]
            if stage128_profile
            else None
        ),
