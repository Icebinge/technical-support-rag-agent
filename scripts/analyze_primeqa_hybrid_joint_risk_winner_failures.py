...
app = typer.Typer(help="Run the Stage 201 train-only joint risk/winner failure attribution.")
...
def main(
    ...
    recovery_context_path: Annotated[Path | None, typer.Option("--recovery-context")] = None,
) -> None:
    settings = ProjectSettings()
    artifacts = settings.artifact_dir
    dependency_dir = artifacts / "stage194_dependency"
    output_path = (
        output or artifacts / "primeqa_hybrid_joint_risk_winner_failure_attribution_stage201.json"
    )
    preflight_failure_path = artifacts / "stage201_corrected_rerun_preflight_failure.json"
    recovery_context = (
        _load_recovery_context(recovery_context_path) if recovery_context_path is not None else None
    )
    report = analysis.run_stage201_joint_risk_winner_failure_attribution(
        stage200_protocol_path=artifacts
        / "primeqa_hybrid_joint_risk_winner_failure_attribution_protocol_stage200.json",
        ...
        preflight_failure_sink=lambda failure: analysis.write_stage201_preflight_failure(
            report=failure, output_path=preflight_failure_path
        ),
        recovery_context=recovery_context,
    )
    report = analysis.write_stage201_report_bundle(
        report=report,
        output_path=output_path,
        visualization_dir=visualization_dir
        or artifacts / "primeqa_hybrid_joint_risk_winner_failure_attribution_stage201_visuals",
    )
    typer.echo(json.dumps(_summary(report), ensure_ascii=True, indent=2))
    typer.echo(f"Saved Stage 201 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "stage": report["stage"],
        "stage199_reproduction": report["stage199_reproduction"],
        "population": attribution["population"],
        "near_boundary_contract": attribution["near_boundary_contract"],
        "constraint_attribution": attribution["constraint_attribution"],
        "fold_attribution": attribution["fold_attribution"],
        "question_context_aggregate": attribution["question_context_attribution"]["aggregate"],
        "factor_attribution": attribution["factor_attribution"],
        "diagnostic_finding": attribution["diagnostic_finding"],
...
def _load_recovery_context(path: Path) -> Mapping[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, Mapping):
        raise ValueError(f"Expected recovery-context JSON object: {path}")
    return value
