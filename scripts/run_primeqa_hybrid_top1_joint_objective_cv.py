...
app = typer.Typer(help="Run the Stage 203 train-only grouped Top-1 objective nested CV.")
...
def main(
    ...
    output_path = output or artifacts / "primeqa_hybrid_top1_joint_objective_stage203.json"
    preflight_failure_path = artifacts / "stage203_stage182_reproduction_failure.json"
    report = analysis.run_stage203_top1_joint_objective_cv(
        ...
        preflight_failure_sink=lambda failure: analysis.write_stage203_preflight_failure(
            report=failure,
            output_path=preflight_failure_path,
        ),
    )
    report = analysis.write_stage203_report_bundle(
        report=report,
        output_path=output_path,
        visualization_dir=visualization_dir
        or artifacts / "primeqa_hybrid_top1_joint_objective_stage203_visuals",
    )
    typer.echo(json.dumps(_summary(report), ensure_ascii=True, indent=2))
    typer.echo(f"Saved Stage 203 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ablation_family_aggregates": cv["ablation_family_aggregates"],
        "safety_weight_aggregates": cv["safety_weight_aggregates"],
        "precision_weight_aggregates": cv["precision_weight_aggregates"],
        "directional_penalty_response": cv["directional_penalty_response"],
