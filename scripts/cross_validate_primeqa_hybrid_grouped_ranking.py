...
app = typer.Typer(help="Run Stage175 train-only grouped ranking nested CV.")
...
def main(
    ...
    """Compare two ranking objectives in 50 grouped train-only fits."""

    settings = ProjectSettings()
    artifacts = settings.artifact_dir
    split_dir = artifacts / "primeqa_hybrid_split_stage68_splits"
    report = analysis.run_stage175_grouped_ranking_cv(
    ...
    visual_dir = visualization_dir or (
        artifacts / "primeqa_hybrid_grouped_ranking_cv_stage175_visuals"
    )
    ...
    output_path = output or (artifacts / "primeqa_hybrid_grouped_ranking_cv_stage175.json")
    ...
    typer.echo(f"Saved Stage175 grouped ranking nested CV: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stage174_comparison": report["stage174_comparison"],
