...
app = typer.Typer(help="在 PrimeQA TechQA 数据集上评估 BM25 检索 baseline。")
...
def main(
    ...
    top_k: Annotated[
        ...
        typer.Option(
            "--top-k",
            help="用逗号分隔的 top-k 取值，例如：1,5,10。",
        ),
    ] = "1,5,10",
    output: Annotated[
        Path | None,
        typer.Option(
            "--output",
            help="可选 JSON 报告路径。默认写入 artifacts/bm25_<split>_metrics.json。",
        ),
    ] = None,
    k1: Annotated[
        float,
    ...
    """运行 PrimeQA BM25 检索评估并保存可复现实验报告。"""
    ...
    output_path = output or settings.artifact_dir / f"bm25_{split}_metrics.json"
    ...
    typer.echo(f"Saved BM25 report: {output_path}")
