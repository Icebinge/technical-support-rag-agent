...
app = typer.Typer(help="Run the Stage 206 train-only two-stage change-ranker nested CV.")
...
def main(
    ...
    output_path = output or artifacts / "primeqa_hybrid_two_stage_change_ranker_stage206.json"
    preflight_failure_path = artifacts / "stage206_stage182_reproduction_failure.json"
    report = analysis.run_stage206_two_stage_change_ranker_cv(
        stage205_protocol_path=artifacts
        / "primeqa_hybrid_two_stage_change_ranker_protocol_stage205.json",
        ...
        preflight_failure_sink=lambda failure: analysis.write_stage206_preflight_failure(
            report=failure,
            output_path=preflight_failure_path,
        ),
    )
    report = analysis.write_stage206_report_bundle(
        report=report,
        output_path=output_path,
        visualization_dir=visualization_dir
        or artifacts / "primeqa_hybrid_two_stage_change_ranker_stage206_visuals",
    )
    typer.echo(json.dumps(_summary(report), ensure_ascii=True, indent=2))
    typer.echo(f"Saved Stage 206 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ranker_family_aggregates": cv["ranker_family_aggregates"],
        "coverage_aggregates": cv["coverage_aggregates"],
        "selected_spec_counts": cv["selected_spec_counts"],
        "selected_ranker_family_counts": cv["selected_ranker_family_counts"],
