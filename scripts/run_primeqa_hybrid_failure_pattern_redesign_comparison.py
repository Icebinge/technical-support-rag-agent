...
app = typer.Typer(
    help="Run Stage109 PrimeQA hybrid failure-pattern redesign train-CV/dev comparison."
)


@app.command()
def main(
    stage108_protocol: Annotated[
        Path | None,
        typer.Option("--stage108-protocol", help="Stage108 frozen protocol JSON."),
    ] = None,
    ...
    output: Annotated[
        Path | None,
        typer.Option("--output", help="Output Stage109 comparison JSON path."),
    ] = None,
    ...
    user_confirmed_comparison: Annotated[
        ...
        typer.Option(
            ...
            help="Required confirmation for the Stage109 train-CV/dev comparison.",
        ),
    ] = False,
    ...
    retrieval_top_k: Annotated[
        int,
        typer.Option("--retrieval-top-k", help="BM25 retrieval depth."),
    ] = 10,
    ...
    sample_limit_per_transition: Annotated[
        int,
        typer.Option(
            "--sample-limit-per-transition",
            help="Public-safe changed-case samples per baseline-to-candidate transition.",
        ),
    ] = 5,
) -> None:
    """Write the Stage109 train-CV-selected, dev-validated comparison report."""

    settings = ProjectSettings()
    split_dir = settings.artifact_dir / "primeqa_hybrid_split_stage68_splits"
    stage108_protocol_path = stage108_protocol or (
    ...
    visualization_output_dir = visualization_dir or (
        settings.artifact_dir
        / "primeqa_hybrid_failure_pattern_redesign_comparison_stage109_visuals"
    )

    report = run_primeqa_hybrid_failure_pattern_redesign_comparison(
        stage108_protocol_path=stage108_protocol_path,
    ...
    typer.echo(f"Saved PrimeQA hybrid Stage109 comparison: {output_path}")
