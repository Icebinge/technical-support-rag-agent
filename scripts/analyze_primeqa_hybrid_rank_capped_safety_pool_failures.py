...
app = typer.Typer(help="Run the Stage 192 train-only within-pool failure attribution.")
...
def main(
    ...
    report = analysis.run_stage192_rank_capped_safety_pool_failure_attribution(
        stage191_report_path=artifacts / "primeqa_hybrid_rank_capped_safety_pool_stage191.json",
        stage190_protocol_path=artifacts
        / "primeqa_hybrid_rank_capped_safety_pool_protocol_stage190.json",
    ...
    visualizations = analysis.write_stage192_visualizations(
        ...
        or artifacts / "primeqa_hybrid_rank_capped_safety_pool_failure_stage192_visuals",
    )
    report = {
        **report,
    ...
    output_path = (
        output or artifacts / "primeqa_hybrid_rank_capped_safety_pool_failure_stage192.json"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(report, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
    typer.echo(json.dumps(_summary(report), ensure_ascii=True, indent=2))
    typer.echo(f"Saved Stage 192 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "stage": report["stage"],
        "stage191_reproduction": report["stage191_reproduction"],
        "reference_trajectory": attribution["reference_trajectory"],
