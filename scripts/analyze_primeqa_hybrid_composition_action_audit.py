...
from ts_rag_agent.application import primeqa_hybrid_composition_action_audit as analysis
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(help="Run the Stage181 train-only counterfactual composition action audit.")
...
def main(
    ...
    report = analysis.run_stage181_composition_action_audit(
    ...
    visuals = analysis.write_stage181_visualizations(
        report=report,
        output_dir=visualization_dir
        or artifacts / "primeqa_hybrid_composition_action_audit_stage181_visuals",
    )
    report = {
        **report,
        "visualizations": [
    ...
    output_path = output or artifacts / "primeqa_hybrid_composition_action_audit_stage181.json"
    ...
    typer.echo(f"Saved Stage181 report: {output_path}")
    if report["decision"]["status"] == "stage181_counterfactual_action_audit_invalid":
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "stage": report["stage"],
        "composition_dataset": report["composition_dataset"],
        "action_audit": report["action_audit"],
        "stage180_selected_action_audit": report["stage180_selected_action_audit"],
        "oof_predictability": report["oof_predictability"],
