...
from ts_rag_agent.application.primeqa_hybrid_sidecar_agent_orchestrator_validation import (
    run_primeqa_hybrid_sidecar_agent_orchestrator_validation,
    write_primeqa_hybrid_sidecar_agent_orchestrator_validation_visualizations,
)
from ts_rag_agent.config import ProjectSettings

app = typer.Typer(help="Run Stage137 Stage116-control vs sidecar-agent train-CV/dev validation.")


@app.command()
def main(
    stage136_protocol: Annotated[
        Path | None,
        typer.Option("--stage136-protocol", help="Stage136 orchestrator protocol JSON."),
    ] = None,
    stage135_validation: Annotated[
        Path | None,
        typer.Option("--stage135-validation", help="Stage135 observation validation JSON."),
    ] = None,
    ...
    output: Annotated[
        Path | None,
        typer.Option("--output", help="Output Stage137 public-safe report JSON."),
    ] = None,
    visualization_dir: Annotated[
        Path | None,
        typer.Option("--visualization-dir", help="Output directory for Stage137 SVG charts."),
    ] = None,
    user_confirmed_validation: Annotated[
        bool,
        typer.Option(
            "--user-confirmed-validation/--no-user-confirmed-validation",
            help="Required confirmation for Stage137 train/dev validation.",
        ),
    ] = False,
    ...
    """Write the Stage137 train-CV/dev public-safe agent validation report."""

    settings = ProjectSettings()
    split_dir = settings.artifact_dir / "primeqa_hybrid_split_stage68_splits"
    stage136_protocol_path = stage136_protocol or (
    ...
    visualization_output_dir = visualization_dir or (
        settings.artifact_dir
        / "primeqa_hybrid_sidecar_agent_orchestrator_validation_stage137_visuals"
    )

    report = run_primeqa_hybrid_sidecar_agent_orchestrator_validation(
    ...
    visualizations = write_primeqa_hybrid_sidecar_agent_orchestrator_validation_visualizations(
        report=report,
    ...
    typer.echo(f"Saved PrimeQA hybrid Stage137 agent validation: {output_path}")
