...
app = typer.Typer(help="Run the Stage 191 train-only rank-capped safety-pool nested CV.")
...
def main(
    ...
    report = analysis.run_stage191_rank_capped_safety_pool_cv(
    ...
    visualizations = analysis.write_stage191_visualizations(
        ...
        or artifacts / "primeqa_hybrid_rank_capped_safety_pool_stage191_visuals",
    )
    report = {
        **report,
        "visualizations": [
    ...
    output_path = output or artifacts / "primeqa_hybrid_rank_capped_safety_pool_stage191.json"
    ...
    typer.echo(f"Saved Stage 191 report: {output_path}")
...
def _summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "aggregate_pool_metrics": cv["aggregate_pool_metrics"],
        "aggregate": cv["aggregate"],
        "paired_bootstrap": cv["paired_bootstrap"],
        "prediction_metrics": cv["prediction_metrics"],
        "selected_pool_cap_counts": cv["selected_pool_cap_counts"],
        "selected_ranker_counts": cv["selected_ranker_counts"],
