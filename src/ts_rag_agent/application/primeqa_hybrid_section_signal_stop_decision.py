...
_STOPPED_PROTOCOL_ID = "section_signal_guarded_expansion_train_dev_v1"
...
_STOP_DECISION_ROUTE_ID = "section_signal_stop_decision"


@dataclass(frozen=True)
class PrimeQAHybridSectionSignalStopVisualization:
    """One generated Stage93 section-signal stop-decision chart."""

    name: str
    path: str


def decide_primeqa_hybrid_section_signal_stop(
    *,
    stage84_report_path: Path,
    stage92_report_path: Path,
    user_confirmed_stop: bool,
    confirmation_note: str,
) -> dict[str, Any]:
    """Stop section-signal guarded expansion after it fails the train/dev gate."""

    started_at = time.perf_counter()
    stage84_report = _load_json_object(stage84_report_path)
    stage92_report = _load_json_object(stage92_report_path)
    loaded_at = time.perf_counter()

    stage84_candidate = _candidate_summary(stage84_report, _STOPPED_CANDIDATE_ID)
    stage92_summary = _stage92_summary(stage92_report)
    candidate_queue = _candidate_queue(stage84_report)
    guard_checks = _guard_checks(
        stage84_report=stage84_report,
        stage92_report=stage92_report,
        stage84_candidate=stage84_candidate,
        stage92_summary=stage92_summary,
    ...
    return {
        ...
        "decision_scope": (
            "Train/dev-only stop decision for the Stage84 section signal "
            "guarded expansion route after Stage92 validation. This stage "
            "reads public-safe Stage84 and Stage92 reports, does not load "
        ...
        "source_files": {
            ...
            "stage92_report": _fingerprint(stage92_report_path),
        },
        "stopped_route": {
            ...
            "stage92_summary": stage92_summary,
            "stop_reason": (
                "The train-selected section-signal config executed gated "
                "promotion actions, but dev hit@10 did not improve and dev "
                "search-depth net improvement remained zero. Stage91 required "
                "dev hit@10 improvement and positive search-depth net "
                "improvement, so the route is non-advancing."
            ),
        },
        "candidate_queue": candidate_queue,
...
def write_primeqa_hybrid_section_signal_stop_visualizations(
    report: Mapping[str, Any],
    output_dir: Path,
) -> list[PrimeQAHybridSectionSignalStopVisualization]:
    """Write SVG charts for Stage93 section-signal stop decision."""

    output_dir.mkdir(parents=True, exist_ok=True)
    charts = {
        "stage93_section_signal_train_dev_hit10_delta.svg": (
            render_horizontal_bar_chart_svg(
                title="Stage93 section signal train vs dev hit@10 delta",
        ...
        "stage93_section_signal_dev_change_counts.svg": (
            render_horizontal_bar_chart_svg(
                title="Stage93 section signal dev changed-case counts",
        ...
        "stage93_second_wave_remaining_candidate_priority.svg": (
            render_horizontal_bar_chart_svg(
                title="Stage93 remaining second-wave candidate priority",
        ...
        "stage93_section_signal_stop_decision_flags.svg": (
            render_horizontal_bar_chart_svg(
                title="Stage93 section signal stop decision flags",
        ...
        "stage93_section_signal_stop_guard_check_status.svg": (
            render_horizontal_bar_chart_svg(
                title="Stage93 section signal stop guard checks",
    ...
    for filename, svg in charts.items():
        ...
        artifacts.append(
            PrimeQAHybridSectionSignalStopVisualization(
                name=filename,
                path=str(path),
            )
        )
...
def _stage92_summary(stage92_report: Mapping[str, Any]) -> dict[str, Any]:
    decision = stage92_report.get("decision") or {}
    train_selection = stage92_report.get("train_selection") or {}
    train_comparison = (
    ...
    config = stage92_report.get("config") or {}
    return {
        ...
        "train_search_depth_net_improvement_count": int(
            train_comparison.get("search_depth_net_improvement_count") or 0
        ),
        ...
        "train_section_signal_promotion_count": int(
            train_comparison.get("section_signal_promotion_count") or 0
        ),
        "dev_hit10_delta": _float_metric(dev_comparison, "hit@10_delta"),
        "dev_search_depth_net_improvement_count": int(
            dev_comparison.get("search_depth_net_improvement_count") or 0
        ),
        ...
        "dev_section_signal_promotion_count": int(
            dev_comparison.get("section_signal_promotion_count") or 0
        ),
        "dev_protected_top10_demotion_count": int(
            dev_comparison.get("protected_top10_demotion_count") or 0
        ),
...
def _guard_checks(
    *,
    stage84_report: Mapping[str, Any],
    stage92_report: Mapping[str, Any],
    stage84_candidate: Mapping[str, Any],
    stage92_summary: Mapping[str, Any],
    ...
    return [
        ...
        _check(
            name="source_stage92_report_is_stage92",
            passed=stage92_report.get("stage") == "Stage 92",
            observed=stage92_report.get("stage"),
            expected="Stage 92",
        ),
        _check(
            name="user_confirmed_stage93_stop_decision",
            passed=user_confirmed_stop,
            observed=user_confirmed_stop,
            expected=True,
        ),
        _check(
            name="stage92_comparison_completed",
            passed=stage92_summary.get("status")
            == "primeqa_hybrid_section_signal_comparison_completed",
            observed=stage92_summary.get("status"),
            expected="primeqa_hybrid_section_signal_comparison_completed",
        ),
        _check(
            name="stage92_candidate_matches_section_signal",
            passed=stage92_summary.get("candidate_id") == _STOPPED_CANDIDATE_ID,
            observed=stage92_summary.get("candidate_id"),
            expected=_STOPPED_CANDIDATE_ID,
        ),
        _check(
            name="stage92_protocol_matches_section_signal",
            passed=stage92_summary.get("protocol_id") == _STOPPED_PROTOCOL_ID,
            observed=stage92_summary.get("protocol_id"),
        ...
        _check(
            name="stage84_candidate_metric_contract_requires_search_depth_gain",
            passed=_requires_search_depth_gain(stage84_candidate),
            observed=stage84_candidate.get("target_metric_contract"),
            expected="secondary metric requires positive search-depth net",
        ),
        _check(
            name="stage92_primary_contract_failed",
            passed=stage92_summary.get("primary_contract_passed") is False,
            observed=stage92_summary.get("primary_contract_passed"),
            expected=False,
        ),
        _check(
            name="stage92_secondary_contract_failed",
            passed=stage92_summary.get("secondary_contract_passed") is False,
            observed=stage92_summary.get("secondary_contract_passed"),
            expected=False,
        ),
        _check(
            name="stage92_guard_contract_passed",
            passed=stage92_summary.get("guard_contract_passed") is True,
            observed=stage92_summary.get("guard_contract_passed"),
            expected=True,
        ),
        _check(
            name="stage92_train_selected_config_has_no_dev_hit10_gain",
            passed=float(stage92_summary.get("dev_hit10_delta") or 0.0) <= 0.0,
            observed=stage92_summary.get("dev_hit10_delta"),
            expected="<= 0.0",
        ),
        _check(
            name="stage92_dev_search_depth_net_not_positive",
            passed=int(stage92_summary.get("dev_search_depth_net_improvement_count") or 0)
            <= 0,
            observed=stage92_summary.get("dev_search_depth_net_improvement_count"),
            expected="<= 0",
        ),
        _check(
            name="stage92_dev_top10_net_not_positive",
            passed=(
                int(stage92_summary.get("dev_top10_improvement_count") or 0)
                - int(stage92_summary.get("dev_top10_regression_count") or 0)
            )
            <= 0,
            observed={
                "improvements": stage92_summary.get("dev_top10_improvement_count"),
                "regressions": stage92_summary.get("dev_top10_regression_count"),
            },
            expected="net <= 0",
        ),
        _check(
            name="stage92_final_test_metrics_locked",
            passed=stage92_summary.get("can_run_final_test_metrics_now") is False,
            observed=stage92_summary.get("can_run_final_test_metrics_now"),
            expected=False,
        ),
        _check(
            name="stage92_final_test_gate_closed",
            passed=stage92_summary.get("can_open_final_test_gate_now") is False,
            observed=stage92_summary.get("can_open_final_test_gate_now"),
            expected=False,
        ),
        _check(
            name="stage92_forbids_test_tuning",
            passed=stage92_summary.get("can_use_test_for_tuning") is False,
            observed=stage92_summary.get("can_use_test_for_tuning"),
            expected=False,
        ),
        _check(
            name="stage92_default_runtime_policy_unchanged",
            passed=stage92_summary.get("default_runtime_policy") == "unchanged",
            observed=stage92_summary.get("default_runtime_policy"),
            expected="unchanged",
        ),
        _check(
        ...
        _check(
            name="stage84_next_candidate_available_after_section_signal_stop",
        ...
        _check(
            name="stage93_no_new_retrieval_metrics_run",
            passed=True,
            observed="not_run",
            expected="not_run",
        ),
        _check(
            name="stage93_final_test_metrics_not_run",
            passed=True,
            observed="not_run",
            expected="not_run",
        ),
        _check(
            name="stage93_default_runtime_policy_unchanged",
            passed=True,
            observed="unchanged",
            expected="unchanged",
        ),
...
def _decision(
    *,
    ...
    if failed_checks:
        return {
            "status": "primeqa_hybrid_section_signal_stop_decision_blocked",
    ...
    return {
        ...
        "recommended_next_stage": (
            "Stage 94: confirm and freeze the train/dev-only protocol for "
...
def _train_dev_delta_bars(report: Mapping[str, Any]) -> list[BarDatum]:
    summary = (report.get("stopped_route") or {}).get("stage92_summary") or {}
    return [
        BarDatum(
...
def _dev_change_bars(report: Mapping[str, Any]) -> list[BarDatum]:
    ...
    return [
        ...
        BarDatum(
            label="dev search-depth net",
            value=float(summary.get("dev_search_depth_net_improvement_count") or 0),
            value_label=str(
                summary.get("dev_search_depth_net_improvement_count") or 0
            ),
        ),
        BarDatum(
            label="dev section-signal promotions",
            value=float(summary.get("dev_section_signal_promotion_count") or 0),
            value_label=str(summary.get("dev_section_signal_promotion_count") or 0),
        ),
    ]
...
def _remaining_candidate_priority_bars(report: Mapping[str, Any]) -> list[BarDatum]:
    ...
    return [
        BarDatum(
            label=str(summary["candidate_id"]),
            value=float(summary.get("priority_score") or 0),
            value_label=str(summary.get("priority_score") or 0),
        )
...
def _decision_flag_bars(report: Mapping[str, Any]) -> list[BarDatum]:
    ...
    return [
        BarDatum(
            label="can_continue_train_dev_development",
            value=float(decision["can_continue_train_dev_development"]),
            value_label="yes" if decision["can_continue_train_dev_development"] else "no",
        ),
        BarDatum(
...
def _requires_dev_hit10_gain(candidate: Mapping[str, Any]) -> bool:
    return any(
        "dev hit@10 must improve" in str(item)
        for item in candidate.get("target_metric_contract") or []
    )


def _requires_search_depth_gain(candidate: Mapping[str, Any]) -> bool:
    return any(
        "search-depth improvements" in str(item)
        for item in candidate.get("target_metric_contract") or []
    )


def _float_metric(row: Mapping[str, Any], key: str) -> float | None:
    value = row.get(key)
    if value is None:
        return None
    return float(value)
